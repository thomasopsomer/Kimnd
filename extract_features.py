...
def get_frequencies_outgoing(df_flat, time):
    ...
    frequencies_recipient = pd.DataFrame(df_flat.groupby(
        ["sender", "recipient"]).mid.nunique()).reset_index()
    # Count the total mails for each user
    frequencies_all = pd.DataFrame(df_flat.groupby(
        "sender").mid.nunique()).reset_index()
    # Rename the columns for better readability
    frequencies_recipient.columns = [
        "sender", "recipient", "OUT_cnt_mess_recipient"]
    frequencies_all.columns = ["sender", "OUT_cnt_mess_all"]
    # Perform an inner join on the column "sender" between the 2 dataframes
    frequencies = frequencies_recipient.merge(
        frequencies_all, how="inner", on="sender")
    # OUT_ratio is the desired final feature
    frequencies["OUT_ratio"] = frequencies[
        "OUT_cnt_mess_recipient"] / frequencies["OUT_cnt_mess_all"]
    # user and contact as in the paper
    frequencies = frequencies.rename(
        columns={"sender": "user", "recipient": "contact"})
...
def get_frequencies_incoming(df_flat, time):
    ...
    if df_flat.empty:
        return pd.DataFrame([])
    frequencies_sender = pd.DataFrame(df_flat.groupby(
        ["recipient", "sender"]).mid.nunique()).reset_index()
    frequencies_all = pd.DataFrame(df_flat.groupby(
        "recipient").mid.nunique()).reset_index()
    frequencies_sender.columns = ["recipient", "sender", "IN_cnt_mess_sender"]
    frequencies_all.columns = ["recipient", "IN_cnt_mess_all"]
    frequencies = frequencies_sender.merge(
        frequencies_all, how="inner", on="recipient")
    frequencies["IN_ratio"] = frequencies[
        "IN_cnt_mess_sender"] / frequencies["IN_cnt_mess_all"]
    frequencies = frequencies.rename(
        columns={"recipient": "user", "sender": "contact"})
...
def get_last_time(df_flat):
    ...
    time_last_sent = df_flat.groupby(
        ["sender", "recipient"]).time.max().reset_index()
...
def get_more_recent_perc_out(df_flat, time):
    ...
    alpha = 2
    # For each couple (user, sender) extract the last time a mail was sent
    # between them
    ...
    cnt_all_mess = pd.DataFrame(df_flat.groupby(
        "sender").mid.nunique()).reset_index()
    ...
    # The desired feature
    more_recents["OUT_ratio_recent"] = more_recents[
        "OUT_cnt_mess_recent"] / (alpha * more_recents["OUT_cnt_mess_all"])
    more_recents = more_recents.rename(
        columns={"sender": "user", "recipient": "contact", "last_time": "OUT_last_time"})
    # Take only the columns that we want
    more_recents = more_recents[
        ["user", "contact", "OUT_last_time", "OUT_ratio_recent"]]
    return more_recents
...
def get_more_recent_perc_in(df_flat, time):
    ...
    more_recents["IN_ratio_recent"] = more_recents[
        "IN_cnt_mess_recent"] / (alpha * more_recents["IN_cnt_mess_all"])
    more_recents = more_recents.rename(
        columns={"recipient": "user", "sender": "contact", "last_time": "IN_last_time"})
    more_recents = more_recents[
        ["user", "contact", "IN_last_time", "IN_ratio_recent"]]
    return more_recents
